...
If you have saves from the original flat `src/` layout:

1. Saves in `Stock-Market-Sim/src/saves/{username}` are detected automatically
2. Encryption keys migrate from `{username}_key` to `data/keys/{username}.key`
3. Save files move to `data/saves/{username}.enc` on next write

Run the new entry point — no manual migration steps required.
